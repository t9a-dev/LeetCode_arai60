...
/*
  ...
  何を考えて解いていたか
  - BFSで根から左優先探索で改装を下りながら配列にノードの値を詰めていくイメージ

  想定ユースケース
  - 組織図などである階層の組織を頻繁かつ高速に取得したい場面。
  一度このメソッドで配列に変換してキャッシュしておけば、ある階層のノード（部署）を時間計算量O(1)のインデックスアクセスですべて取得できる。

  正解してから気づいたこと
  - match文である位置に既に配列が存在すればノードの値を追加する。なければ配列と値を追加する。としている箇所が、HashMapにおけるentry().and_modify().or_inser()に似ているななどと思った。
    - HashMapを利用する際に払うハッシュ化関数のコストを削減できるので、問題によってはこの考え方でHashMapを二次元配列に置き換えられるななどと考えた。
  - 再帰によるDFS、iterable DFSも練習として実装しておく。
*/
