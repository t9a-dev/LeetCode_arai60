...
/*
  ...
  何がわからなかったか
  - 階層ごとに優先探索方向（左右）を切り替える方法だと通らないテストケースがある。
    [[1],[3,2],[4,5]]
      1
     / \
    2   3
  ...
  何を考えて解いていたか
  - BFSで階層ごとに幅優先探査を行う。左側優先探索から開始して階層が奇数のときに右側優先探索に切り替える。
  - BFSで階層ごとにやると親ノードが異なるとうまく行かない。階層ごとにノードの値を入れておく配列をVecDequeにして、ジグザグかどうかでpush_back(),push_front()を切り替える方法が必要。

  正解してから気づいたこと
  - 最初に思いついた階層ごとにfrontiersに入れる順番を入れ替える解法では親ノードが共通のノードでしかジグザグにできないことに気付くのに時間がかかった。
  - VecDequeからVecへの変換が初めてだったので少し手間取った。
  - 再帰DFS,iterable DFSも練習しておいた方が良さそう。
*/
